...
class Learning:
    ...
    def generate_sample2(self, n):
        ...
        for s in self.sources:
            ...
            # features[s] = np.absolute(features[s])
    ...
    def distance(self, Y1, Y2):
        ...
        for (s, t) in self.paths:
            for p in range(len(self.paths[(s, t)])):
                distance += abs(Y1[(s, t)][p] - Y2[(s, t)][p])
        return distance
...
class FrankWolf(Gradient):
    ...
    def find_max_distributed(self, Z, iterations, stepsize):
        ...
        for i in range(iterations):
            ...
            for e in self.dependencies:
                ...
                temp_Q[e] = temp
                temp = np.expm1(temp)
                Q_new[e] += stepsize / (i + 1) ** 0.0 * self.plus(Q_old[e], temp)
            ...
            for (s, t) in self.paths:
                ...
                temp_R[(s, t)] = temp
                temp = np.expm1(temp)
                R_new[(s, t)] += stepsize / (i + 1) ** 0.0 * self.plus(R_old[(s, t)], temp)

            for (s, t) in self.paths:
                for p in range(len(self.paths[(s, t)])):
                    U_new[(s, t)][p] = U_old[(s, t)][p] + stepsize / (i + 1) ** 0.0 * self.plus(U_old[(s, t)][p],
                    ...
                    D_new[(s, t)][p] = D_old[(s, t)][p] + stepsize / (i + 1) ** 0.0 * (
            ...
            # L = self.Lagrangian(Z, D_new, Q_new, R_new, U_new, n)
            # print(L)
    ...
    def alg(self, iterations, head, N1, N2, stepsize):
        ...
        for t in range(iterations):
            ...
            logging.debug("optimal value: " + str(obj))
            D2 = self.find_max(Z2)
        ...
        for i in range(iterations):
            ...
            for (s, t) in self.paths:
                for p in range(len(self.paths[(s, t)])):
                    ...
                    D_new[(s, t)][p] = D_old[(s, t)][p] + stepsize / (i + 1) ** 0.0 * (temp_sum ** (-alpha) - temp -
                                                                      R_old[(s, t)] + U_old[(s, t)][p])
